...
public interface ContactTracingService {
     ...
     */
    Task<Integer> startContactTracing(PendingIntent contactTracingCallback);

    @IntDef({Status.SUCCESS, Status.FAILED_REJECTED_OPT_IN, Status.FAILED_SERVICE_DISABLED, Status.FAILED_BLUETOOTH_SCANNING_DISABLED, Status.FAILED_TEMPORARILY_DISABLED, Status.FAILED_INSUFFICENT_STORAGE, Status.FAILED_INTERNAL})
    @interface Status {
        int SUCCESS = 0;
    ...
    interface ContactTracingCallback {
        ...
        // least 24 hours old.
        //
        // In order to be whitelisted to use this API, apps will be required to timestamp Information
     ...
     */
    Task<Integer> stopContactTracing();

    /**
     * Indicates whether contact tracing is currently running for the
     * requesting app.
     */
    Task<Integer> isContactTracingEnabled();
     ...
     */
    Task<Integer> startSharingDailyTracingKeys();
     ...
     */
    Task<Integer> provideDiagnosisKeys(List<DailyTracingKey> keys);
